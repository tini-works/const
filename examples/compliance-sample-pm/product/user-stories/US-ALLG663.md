## US-ALLG663 — Contract documents marked as "Public" must be printable at any...

| Field | Value |
|-------|-------|
| **ID** | US-ALLG663 |
| **Traced from** | [ALLG663](../compliances/SV/ALLG663.md) |
| **Source** | AKA Q1-26-1 |
| **Status** | TBC |
| Matched by | — |
| Proven by | — |
| Confirmed by | — |
| **Epic** | [E6: General & Documentation](../epics/E6-general-and-documentation.md) |
| **Data Entity** | VTG |

### User Story

As a practice owner, I want contract documents marked as "Public" is printable at any time, so that general compliance requirements are met.

### Acceptance Criteria

1. Given a public Vertragsdokument, when print is requested, then it prints successfully

### Actual Acceptance Criteria

| Status | **Not Implemented** |
|--------|-------------------|

1. No print functionality for contract documents was found in the backend codebase.
2. While the system can retrieve contract attachments and documents, there is no print endpoint or print-formatting service for public Vertragsdokumente.
3. **Gap**: The requirement to print public contract documents at any time is not implemented.
