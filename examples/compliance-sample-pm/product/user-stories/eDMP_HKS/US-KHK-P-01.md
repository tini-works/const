...
1. **API Coverage**: The `EDMPApp.CheckPlausibility` endpoint (NATS topic `api.app.app_core.EDMPApp.CheckPlausibility`) applies all cross-program plausibility rules (EXT_ITA_VGEX_Plausi_eDMP_Uebergreifend) when `DMPLabelingValue = "KHK"` is specified.
