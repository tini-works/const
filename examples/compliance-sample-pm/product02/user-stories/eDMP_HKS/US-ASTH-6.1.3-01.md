## US-ASTH-6.1.3-01 — Practice software must encode clinical data using sciphox:sciphox-ssu observation structure

| Field | Value |
|-------|-------|
| **ID** | US-ASTH-6.1.3-01 |
| **Traced from** | [ASTH-6.1.3-01](../../compliances/raw/eDMP-HKS/eDMP-HKS/Asthma/ASTH-6.1.3-01.md) |
...
As a practice software, I want to encode all clinical data fields using the sciphox:sciphox-ssu observation structure, so that data values are machine-readable and conform to the eDMP XML schema.

### Acceptance Criteria

1. Given clinical data is documented for eDMP Asthma, when the XML is generated, then each data field is wrapped in a sciphox:sciphox-ssu observation element
2. Given the observation structure is missing or malformed, when validated, then an error is reported

### Actual Acceptance Criteria

1. **API Coverage**: The `EDMPApp.CreateDocument` endpoint encodes all Asthma clinical data fields using the sciphox:sciphox-ssu observation structure when generating the XML document.
