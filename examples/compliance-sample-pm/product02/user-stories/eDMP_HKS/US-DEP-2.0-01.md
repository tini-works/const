...
1. **API Coverage**: The `EDMPApp.CreateDocument` endpoint (NATS topic `api.app.app_core.EDMPApp.CreateDocument`) generates Depression export files with KBV-conformant naming when `DMPLabelingValue = "DEPRESSION"` is specified in the `DocumentationOverview`.
...
3. **Integration Verification**: Call `EDMPApp.FinishDocumentationOverview` with a complete Depression documentation overview and verify the returned `CheckPlausibilityResponse.billingFile` has a correctly-named file.
