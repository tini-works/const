...
1. **API Coverage**: The `EDMPApp.CreateDocument` endpoint (NATS topic `api.app.app_core.EDMPApp.CreateDocument`) generates Diabetes mellitus Typ 2 export files with KBV-conformant naming when `DMPLabelingValue = "DM2"` is specified in the `DocumentationOverview`.
...
3. **Integration Verification**: Call `EDMPApp.FinishDocumentationOverview` with a complete Diabetes mellitus Typ 2 documentation overview and verify the returned `CheckPlausibilityResponse.billingFile` has a correctly-named file.
