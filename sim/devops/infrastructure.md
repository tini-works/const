...
Not on the critical path. This service can be stopped during peak clinic hours without affecting check-in operations. Run migration batches during off-hours (after 6 PM).
...
Failure mode: if Google Vision is unavailable, OCR requests fail gracefully. Patients fall back to manual entry. OCR is an accelerator, not a gatekeeper.
...
Application services connect to PgBouncer on port 6432, not directly to PostgreSQL on 5432.
